...
presence = r"msculpturalis_occurrence_20230925"
cost = r"costs_with_barriers"
...
print(f"[{dt.now().strftime('%H:%M:%S')}] Raster has CRS EPSG:{crs} and cell size {cell_size}.")
...
def thinning():
    ...
    print(f"[{dt.now().strftime('%H:%M:%S')}] Projecting presence data to EPSG:{crs}...")
    ...
    thinned.to_file(os.path.join(workdir, f"{presence}_thinned.gpkg"), layer=f"{presence}_thinned", driver="GPKG")
    ...
    if arcpy.Exists(os.path.join(gdb, f"{presence}_thinned")):
        arcpy.Delete_management(os.path.join(gdb, f"{presence}_thinned"))

    arcpy.CopyFeatures_management(os.path.join(workdir, f"{presence}_thinned.gpkg", f"{presence}_thinned"),
                                  os.path.join(gdb, f"{presence}_thinned"))

    # Remove the GeoPackage file if no longer needed
    os.remove(os.path.join(workdir, f"{presence}_thinned.gpkg"))
...
def distacc():
    ...
    try:
        ...
        for year in range(start_year, end_year + 1, 1):
            ...
            path_dist_acc_raster = os.path.join(gdb, f"{presence}_{year}_acc")
            path_back_dir_raster = os.path.join(gdb, f"{presence}_{year}_back")

            # Select occurrences known in the respective year.
            arcpy.management.MakeFeatureLayer(os.path.join(gdb, f"{presence}_thinned"), "temp_layer", f"observation_year <= {year}")
...
def optpaths():
    ...
    try:
        ...
        for idx, year in enumerate(range(start_year + 1, end_year + 1)):
            ...
            path_dist_acc_raster = os.path.join(gdb, f"{presence}_{year - 1}_acc")
            path_back_dir_raster = os.path.join(gdb, f"{presence}_{year - 1}_back")
            ...
            if idx == 0:
                ...
                arcpy.management.CopyFeatures(path_optpaths, os.path.join(gdb, f"{presence}_paths"))
            else:
                print(f"[{dt.now().strftime('%H:%M:%S')}] Appending optimal paths for year {year}...")
                arcpy.management.Append(inputs=[path_optpaths], target=os.path.join(gdb, f"{presence}_paths"))

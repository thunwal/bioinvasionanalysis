...
header-includes:
  ...
  \fancypagestyle{plain}{
    \fancyhf{}
    \fancyhead[L]{Notes on the effects of different cost raster input - simple (altitude) vs. reversed SDM}
...
cost_surface_name <- "costs_without_barriers_rescaled100.tif"
paths_gpkg <- "msculpturalis_20240106_simple100.gpkg" 
paths_layer_name <- "msculpturalis_20240106_simple100_paths"

cost_surface_name_2 <- "msculpturalis_sdm_clip_rev_scaled100.tif"
paths_gpkg_2 <- "msculpturalis_20240106_sdmrev100.gpkg" 
paths_layer_name_2 <- "msculpturalis_20240106_sdmrev100_paths"
...
lcp$accumulated_cost_log <- log(lcp$accumulated_cost)
...
lcp_2$accumulated_cost_log <- log(lcp_2$accumulated_cost)
...
# Notes on the effects of different cost raster input - simple (altitude) vs. reversed SDM

**Left side:**  
Cost raster *`r cost_surface_name`* with value range `r minmax(cost_surface)[1]`-`r minmax(cost_surface)[2]` at a resolution of 600 m and resulting least-cost paths.  
The cost raster is a rescaled version of the cost raster used in my project work and assumes more friction with higher altitudes.

**Right side:**  
Cost raster *`r cost_surface_name_2`* with value range `r minmax(cost_surface_2)[1]`-`r minmax(cost_surface_2)[2]` at a resolution of 10 km and resulting least-cost paths.  
The cost raster is a reversed and rescaled version of the habitat suitability raster ("consensus map") published by [Lanner et al. (2022)](https://doi.org/10.1016/j.scitotenv.2022.154246).
...
map_cost_2 <- ggplot() +
  ...
  scale_fill_viridis_c(na.value = "transparent", direction = -1, name = "value") +
  #geom_sf(data = countries, fill = "transparent", color = "gray", linewidth = 0.1) +
  geom_sf(data = graticules, color = "gray", linetype = "dashed", linewidth = 0.1) +
  coord_sf(expand = FALSE) +  # this removes white space between map and axes
  ggtitle(cost_surface_name_2)

plot_grid(map_cost, map_cost_2)
```

```{r histograms, results='hide', fig.keep='all', out.width='\\textwidth', out.height='\\textheight', fig.align='top'}
# Initial theme does not plot axis title, enable for histograms
theme_update(axis.title = element_text(size = 6))

# Create histograms with accumulated cost of LCPs
hist_acc_cost <- ggplot(data = lcp, aes(x = accumulated_cost)) + 
  geom_histogram(bins=50) +
  labs(
    title = paste0(paths_layer_name, "\nDistribution of accumulated cost"),
    x = "Accumulated Cost",
    y = "Count"
  )

hist_acc_cost_2 <- ggplot(data = lcp_2, aes(x = accumulated_cost)) + 
  geom_histogram(bins=50) +
  labs(
    title = paste0(paths_layer_name_2, "\nDistribution of accumulated cost"),
    x = "Accumulated Cost",
    y = "Count"
  )

hist_acc_cost_log <- ggplot(data = lcp, aes(x = accumulated_cost_log)) + 
  geom_histogram(bins=50) +
  labs(
    title = paste0(paths_layer_name, "\nDistribution of accumulated cost (log)"),
    x = "Accumulated Cost",
    y = "Count"
  )

hist_acc_cost_log_2 <- ggplot(data = lcp_2, aes(x = accumulated_cost_log)) + 
  geom_histogram(bins=50) +
  labs(
    title = paste0(paths_layer_name_2, "\nDistribution of accumulated cost (log)"),
    x = "Accumulated Cost",
...
plot_grid(hist_acc_cost, hist_acc_cost_2, hist_acc_cost_log, hist_acc_cost_log_2)

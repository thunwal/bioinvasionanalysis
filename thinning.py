...
def thinning(gdb, workdir, presence, run, year_field, desc):
    extent = desc.extent
    ...
    print(
        f"[{dt.now().strftime('%H:%M:%S')}] Raster has CRS factoryCode {crs_code} and cell size {cell_size} x {cell_size_y}.")
    ...
    try:
        ...
        try:
            ...
            raise Exception(
                f"[{dt.now().strftime('%H:%M:%S')}] Failed to project presence data with 'epsg:{crs_code}' and 'esri:{crs_code}'.") from e2

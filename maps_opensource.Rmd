...
header-includes:
  ...
  \fancypagestyle{plain}{
    \fancyhf{}
    \fancyhead[L]{Notes on the effects of different cost raster input - open-source version}
...
gdb_path <- paste0(workdir, "/", "p_distanzanalyse.gdb")
cost_surface_name <- "costs_without_barriers.tif"
cost_surface_name_2 <- "msculpturalis_sdm_clip_rev.tif"
presence_name <- "msculpturalis_20230925"
lcp_name <- "msculpturalis_20230925_costrast1_paths"
lcp_name_2 <- "msculpturalis_20230925_sdmrev_paths"
quantile <- 0.98

# Load data
cost_surface <- rast(paste0(workdir, "/", cost_surface_name))
NAflag(cost_surface) <- 0  # tbd: workaround to avoid NoData being plotted
site_bbox <- ext(cost_surface)

cost_surface_2 <- rast(paste0(workdir, "/", cost_surface_name_2))
...
presence <- st_read(dsn = gdb_path, layer = presence_name)

lcp <- st_read(dsn = gdb_path, layer = lcp_name)
threshold <- quantile(lcp$PathCost, quantile)
lcp_sub <- lcp[lcp$PathCost < threshold, ]

lcp_2 <- st_read(dsn = gdb_path, layer = lcp_name_2)
threshold_2 <- quantile(lcp_2$PathCost, quantile)
lcp_sub_2 <- lcp_2[lcp_2$PathCost < threshold_2, ]
...
# Notes on the effects of different cost raster input - Esri version

**Left side:**  
Cost raster *`r cost_surface_name`* with value range 1-`r minmax(cost_surface)[2]` at a resolution of 600 m and resulting least-cost paths.  
The cost raster is taken from my project work and assumes more friction with higher altitudes.
...
The cost raster is a reversed version of the habitat suitability raster published by [Lanner et al. (2022)](https://doi.org/10.1016/j.scitotenv.2022.154246).

**Methodology:**  
Least-cost paths were computed using Esri ArcGIS Pro's Distance toolset, which is based on a [distortion-free algorithm](https://pro.arcgis.com/en/pro-app/latest/tool-reference/spatial-analyst/distance-accumulation-algorithm.htm).
...
for (year in 2009:2023) {
  ...
  lcp1 <- basemap +
    ...
    coord_sf(expand = FALSE) +
    ggtitle(paste0(year, " | ", lcp_name, " | cost < ", quantile, " quantile (", threshold, ")"))
  ...
  lcp2 <- basemap +
    ...
    coord_sf(expand = FALSE) +
    ggtitle(paste0(year, " | ", lcp_name_2, " | cost < ", quantile, " quantile (", threshold_2, ")"))
